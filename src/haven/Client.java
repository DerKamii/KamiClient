...
public class Client implements Console.Directory {
	...
	if(ed.equals("stderr")) {
	    ...
	    /* KamiClient: we always want the error GUI and the watchdog, even
	     * with no errorurl set — upstream only wires them up when there's
	     * somewhere to send reports to. A null dest just means "don't
	     * send", and errorsent() flags the 137 exit the launcher watches
